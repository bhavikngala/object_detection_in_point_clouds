...
parser.add_argument('--step-lr', action='store_true', description='to set step learing rate or not')
parser.add_argument('--aug-data', action='store_true', description='to augment data or to not')
parser.add_argument('-f', '--model-file', default=None, description='used to set different model file name other than default one')
parser.add_argument('-r', '--root-dir', default=None, description='root directory for data')
parser.add_argument('-p', '--pixor', action='store_true', description='use pixor augmentation scheme')
parser.add_argument('-v', '--voxelnet', action='store_true', description='use voxelnet augmentation scheme')
parser.add_argument('-e', '--epochs', type=int, default=None, description='used to specify number of epochs other than the default value set in config')
parser.add_argument('--aug-scheme', default=None, description='augmentation scheme string used in DataLoader')
parser.add_argument('-m', '--multi-gpu', action='store_true', description='use multiple gpus if set')
parser.add_argument('--val', action='store_true', description='evaluate on validation set during training or not')
parser.add_argument('-c', '--clip', action='store_true', description='whether to clip gradients or not')
parser.add_argument('--clipvalue', type=float, default=0.25, description='gradient norm value threshold for clipping')
parser.add_argument('--resnet18', action='store_true', description='use standard resent 18 as backbone')
...
def train(epoch):
	...
	for batchId, batch_data in enumerate(train_loader):
		...
		if trainLoss is not None and (batchId+1)%cnf.accumulationSteps == 0:
